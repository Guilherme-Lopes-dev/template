      ...
      <a-calendar
        ...
        ><template #dateCellRender="{ current: value }">
          <ul class="events">
            ...
            >
              <template #overlay>
                <ProjectUpdate :onEventDelete="onEventDelete" v-bind="item" :onSection="onEventDelete" />
              </template>
              <li ref="getInput">
      ...
      calenderDom.forEach((element) => {
        element.addEventListener('click', (e) => {
          if (e.target.classList[0] === 'ant-fullcalendar-content') {
            const getDate = dayjs(e.currentTarget.closest('td').getAttribute('title')).format('YYYY-MM-DD');
