      ...
      <div class="calendar-header__left">
        ...
        <div class="calender-head__navigation">
          ...
          <span class="date-label">
            {{ `${dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).format('MMM DD')}` }}
            -
            <template
              v-if="
                parseInt(dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).format('DD'), 10) + 6 <=
                dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).daysInMonth()
              "
              >{{ dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).format('MMM') }}</template
            ><template v-else>{{
              dayjs(
                dayjs()
                  .day('Sunday')
            ...
            >
            <template v-else>{{
              parseInt(dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).format('DD'), 10) +
              6 -
              parseInt(dayjs(dayjs().day('Sunday').year(year).week(currentWeek).toDate()).daysInMonth(), 10)
            }}</template
            >, {{ year }}
          </span>
      ...
      <thead>
        <tr>
          ...
          <th v-for="day in daysOfWeek()" :key="day">
            ...
            <p :class="dayjs().format('MM/DD/YYYY') === day ? 'week-date primary' : 'week-date deactivate'">
        ...
        <tr v-for="time in eventTimes" :key="time">
          ...
          <td
            ...
            :class="`ant-picker-calendar-date-content ${
              dayjs().format('h A') === time && dayjs().format('MM/DD/YYYY') === day ? 'current-data' : null
            }`"
            :title="day"
          >
            <span
              v-if="dayjs().format('h A') === time && dayjs().format('MM/DD/YYYY') === day"
              class="currentTime secondary"
            />

            <template v-for="event in events" :key="event.id">
              <sdDropdown
                v-if="day === event.date[0] && time === dayjs(event.time[0], 'h:mm a').format('h A')"
...
const WeekCalendar = defineComponent({
  ...
  setup() {
      ...
      calenderDom.forEach((element) => {
        element.addEventListener('click', (e) => {
          if (e.target.classList[0] === 'ant-picker-calendar-date-content') {
            defaultValue.value = dayjs(e.currentTarget.getAttribute('title')).format('YYYY-MM-DD');
    ...
    const maximumDate = () => {
      return parseInt(
        dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('DD'),
        10,
      ) +
        6 <=
        dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).daysInMonth()
        ? parseInt(dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('DD'), 10) +
            6
        : parseInt(dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('DD'), 10) +
            6 -
            parseInt(
    ...
    const daysOfMonth = dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).daysInMonth();
    ...
    const daysOfWeek = () => {
      ...
      if (minimumDate() + 6 <= daysOfMonth) {
        for (let i = minimumDate(); i <= maximumDate(); i += 1) {
          days.push(
            `${dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('MM')}/${i}/${
              year.value
            }`,
          );
        }
        ...
        for (let j = 1; j <= minimumDate() + 6 - daysOfMonth; j += 1) {
          days.push(
            `${
              parseInt(
                dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('MM'),
                10,
              ) < 12
                ? parseInt(
                    dayjs(dayjs().day('Sunday').year(year.value).week(currentWeek.value).toDate()).format('MM'),
