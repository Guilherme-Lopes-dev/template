      ...
      <div class="calendar-header__left">
        ...
        <div class="calender-head__navigation">
          ...
          <span class="date-label">
            {{
              `${dayjs().add(currentMonth, 'month').format('MMM YYYY')} - ${dayjs()
    ...
    <table class="table-event schedule-event" width="100%">
      <tbody ref="dataList">
        <template v-for="(date, index) in [...new Set(uniqueDate)]" :key="index + 1">
          <tr
            v-if="
              dayjs(date).format('MM') >= dayjs().add(currentMonth, 'month').format('MM') &&
          ...
          >
            <td class="schedule-time">
              <span class="schedule-date">{{ dayjs(date).format('DD MMM') }}</span>
              <span class="schedule-date-name">{{ dayjs(date).format('ddd') }}</span>
            </td>
